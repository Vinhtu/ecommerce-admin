...
import {
	...
	PutMaterial,
	DeleteMaterial,
} from '../../../redux/actions/utils/materials';

import Select from '../../../components/bootstrap/forms/Select';
import { GetCategorys } from '../../../redux/actions/categorys';

// eslint-disable-next-line react/prop-types
const MaterialComponent = ({ isFluid }) => {
	const dispatch = useDispatch();
	const {
		// themeStatus,
		darkModeStatus,
	} = useDarkMode();
	const { MaterialList, isPutMaterial, isPostMaterial, isDeleteMaterial } = useSelector(
	...
	const [opentNewMaterial, setOpenNewMaterial] = useState(false);
	const [opentEditMaterial, setOpenEditMaterial] = useState(false);
	const [opentDeleteMaterial, setOpenDeleteMaterial] = useState(false);
	...
	useEffect(() => {
		dispatch(GetMaterials());
	}, [dispatch, isPutMaterial, isPostMaterial, isDeleteMaterial]);

	const [currentPage, setCurrentPage] = useState(1);
	const [perPage, setPerPage] = useState(PER_COUNT['5']);

	const { items } = useSortableData(MaterialList ? MaterialList.data.results : Material);
	...
	const deleteMaterial = () => {
		dispatch(DeleteMaterial(dataformedit._id, token));
	};
	...
	const handleChangeMaterialEdit = (e) => {
		setDataFormEdit({ ...dataformedit, [e.target.id]: e.target.value });
	};

	const postMaterial = () => {
		dispatch(PostMaterial(dataform, token));
	};
	useEffect(() => {
		if (isPostMaterial === 'success') {
			setOpenNewMaterial(false);
		}
	}, [isPostMaterial]);

	const editMaterial = () => {
		dispatch(PutMaterial(dataformedit._id, dataformedit, token));
	};

	useEffect(() => {
		if (isPutMaterial === 'success') {
			setOpenEditMaterial(false);
		}
	}, [isPutMaterial]);
	useEffect(() => {
		if (isDeleteMaterial === 'success') {
	...
	return (
		<>
			<Card stretch={isFluid}>
				<CardHeader borderSize={1}>
					<CardLabel icon='Alarm' iconColor='info'>
						<CardTitle>
							Total ' {MaterialList && MaterialList.data.results.length} ' Materials
						</CardTitle>
					</CardLabel>
					<CardActions>
						<Button
			...
			<OffCanvas
				setOpen={setOpenNewMaterial}
				isOpen={opentNewMaterial}
				titleId='New Material'
				isBodyScroll
				placement='end'
				// className='offcanvas-newMaterial'
			>
				<OffCanvasHeader setOpen={setOpenNewMaterial}>
					<OffCanvasTitle id='NewMaterial'>New Material</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						...
						<div className='col-12'>
							<FormGroup id='name' label='Tên tính năng'>
								<Input onChange={handleChangeMaterial} value={dataform.name} />
							</FormGroup>
						</div>
					</div>
				</OffCanvasBody>
				<div className='row m-0'>
					<div className='col-12 p-3'>
						{isPostMaterial === 'isPostMaterial' && (
							<p style={{ fontSize: 14, color: 'red' }}> Tao moi that bai</p>
						)}
						<Button color='info' className='w-100' onClick={() => postMaterial()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				...
				isOpen={opentEditMaterial}
				titleId='Edit Material'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenEditMaterial}>
					<OffCanvasTitle id='EditMaterial'>Edit Material</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
							<FormGroup id='category' label='Danh mục' className='col-md-12'>
								<Select
									...
									placeholder='Choose Category'
									list={selectCategory}
									onChange={handleChangeMaterialEdit}
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						...
						<Button color='info' className='w-100' onClick={() => editMaterial()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				setOpen={setOpenDeleteMaterial}
				isOpen={opentDeleteMaterial}
				titleId='Edit Material'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenDeleteMaterial}>
					<OffCanvasTitle id='EditMaterial'>Delete Material</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						{isDeleteMaterial === 'fail' && (
							<p style={{ fontSize: 14, color: 'red' }}> Delete thất bại</p>
						)}
						<Button color='waring' className='w-100' onClick={() => deleteMaterial()}>
							Delete
						</Button>
					</div>
				</div>
