...
import {
	...
	PutFeature,
	DeleteFeature,
} from '../../../redux/actions/utils/features';

import Select from '../../../components/bootstrap/forms/Select';
import { GetCategorys } from '../../../redux/actions/categorys';

// eslint-disable-next-line react/prop-types
const FeatureComponent = ({ isFluid }) => {
	const dispatch = useDispatch();
	const {
		// themeStatus,
		darkModeStatus,
	} = useDarkMode();
	const { FeatureList, isPutFeature, isPostFeature, isDeleteFeature } = useSelector(
		(state) => state.feature,
	);
	const {
		categoryList,
	...
	const [opentNewFeature, setOpenNewFeature] = useState(false);
	const [opentEditFeature, setOpenEditFeature] = useState(false);
	const [opentDeleteFeature, setOpenDeleteFeature] = useState(false);
	...
	useEffect(() => {
		dispatch(GetFeatures());
	}, [dispatch, isPutFeature, isPostFeature, isDeleteFeature]);

	const [currentPage, setCurrentPage] = useState(1);
	const [perPage, setPerPage] = useState(PER_COUNT['5']);

	const { items } = useSortableData(FeatureList ? FeatureList.data.results : Feature);
	...
	const deleteFeature = () => {
		dispatch(DeleteFeature(dataformedit._id, token));
	};
	...
	const handleChangeFeatureEdit = (e) => {
		setDataFormEdit({ ...dataformedit, [e.target.id]: e.target.value });
	};

	const postFeature = () => {
		dispatch(PostFeature(dataform, token));
	};
	useEffect(() => {
		if (isPostFeature === 'success') {
			setOpenNewFeature(false);
		}
	}, [isPostFeature]);

	const editFeature = () => {
		dispatch(PutFeature(dataformedit._id, dataformedit, token));
	};

	useEffect(() => {
		if (isPutFeature === 'success') {
			setOpenEditFeature(false);
		}
	}, [isPutFeature]);
	useEffect(() => {
		if (isDeleteFeature === 'success') {
	...
	return (
		<>
			<Card stretch={isFluid}>
				<CardHeader borderSize={1}>
					<CardLabel icon='Alarm' iconColor='info'>
						<CardTitle>
							Total ' {FeatureList && FeatureList.data.results.length} ' Features
						</CardTitle>
					</CardLabel>
					<CardActions>
						<Button
								...
								<tr key={item.id}>
									...
									<td>
										...
										<Button
											...
											icon='Edit'
											onClick={() => openDeleteFeature(item)}>
											Delete
										</Button>
									</td>
								</tr>
			...
			<OffCanvas
				setOpen={setOpenNewFeature}
				isOpen={opentNewFeature}
				titleId='New Feature'
				isBodyScroll
				placement='end'
				// className='offcanvas-newFeature'
			>
				<OffCanvasHeader setOpen={setOpenNewFeature}>
					<OffCanvasTitle id='NewFeature'>New Useage Status</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						...
						<div className='col-12'>
							<FormGroup id='name' label='Tên tính năng'>
								<Input onChange={handleChangeFeature} value={dataform.name} />
							</FormGroup>
						</div>
					</div>
				</OffCanvasBody>
				<div className='row m-0'>
					<div className='col-12 p-3'>
						{isPostFeature === 'isPostFeature' && (
							<p style={{ fontSize: 14, color: 'red' }}> Tao moi that bai</p>
						)}
						<Button color='info' className='w-100' onClick={() => postFeature()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				setOpen={setOpenEditFeature}
				isOpen={opentEditFeature}
				titleId='Edit Feature'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenEditFeature}>
					<OffCanvasTitle id='EditFeature'>Edit Useage Status</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
							<FormGroup id='category' label='Danh mục' className='col-md-12'>
								<Select
									...
									placeholder='Choose Category'
									list={selectCategory}
									onChange={handleChangeFeatureEdit}
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						...
						<Button color='info' className='w-100' onClick={() => editFeature()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				...
				isOpen={opentDeleteFeature}
				titleId='Edit Feature'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenDeleteFeature}>
					<OffCanvasTitle id='EditFeature'>Delete Usage Status</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						{isDeleteFeature === 'fail' && (
							<p style={{ fontSize: 14, color: 'red' }}> Delete thất bại</p>
						)}
						<Button color='waring' className='w-100' onClick={() => deleteFeature()}>
							Delete
						</Button>
					</div>
				</div>
