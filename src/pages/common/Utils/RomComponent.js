...
import {
	...
	PutRom,
	DeleteRom,
} from '../../../redux/actions/utils/roms';

import Select from '../../../components/bootstrap/forms/Select';
import { GetCategorys } from '../../../redux/actions/categorys';

// eslint-disable-next-line react/prop-types
const RomComponent = ({ isFluid }) => {
	const dispatch = useDispatch();
	const {
		// themeStatus,
		darkModeStatus,
	} = useDarkMode();
	const { RomList, isPutRom, isPostRom, isDeleteRom } = useSelector(
		(state) => state.rom,
	);
	const {
		categoryList,
	...
	const [opentNewRom, setOpenNewRom] = useState(false);
	const [opentEditRom, setOpenEditRom] = useState(false);
	const [opentDeleteRom, setOpenDeleteRom] = useState(false);
	...
	useEffect(() => {
		dispatch(GetRoms());
	}, [dispatch, isPutRom, isPostRom, isDeleteRom]);

	const [currentPage, setCurrentPage] = useState(1);
	const [perPage, setPerPage] = useState(PER_COUNT['5']);

	const { items } = useSortableData(RomList ? RomList.data.results : Rom);
	...
	const openDeleteRom = (item) => {
		...
		setOpenDeleteRom(true);
	};

	const deleteRom = () => {
		dispatch(DeleteRom(dataformedit._id, token));
	};
	...
	const handleChangeRomEdit = (e) => {
		setDataFormEdit({ ...dataformedit, [e.target.id]: e.target.value });
	};

	const postRom = () => {
		dispatch(PostRom(dataform, token));
	};
	useEffect(() => {
		if (isPostRom === 'success') {
			setOpenNewRom(false);
		}
	}, [isPostRom]);

	const editRom = () => {
		dispatch(PutRom(dataformedit._id, dataformedit, token));
	};

	useEffect(() => {
		if (isPutRom === 'success') {
			setOpenEditRom(false);
		}
	}, [isPutRom]);
	useEffect(() => {
		if (isDeleteRom === 'success') {
			setOpenDeleteRom(false);
		}
	}, [isDeleteRom]);
	...
	return (
		<>
			<Card stretch={isFluid}>
				<CardHeader borderSize={1}>
					<CardLabel icon='Alarm' iconColor='info'>
						<CardTitle>
							Total ' {RomList && RomList.data.results.length} ' Roms
						</CardTitle>
					</CardLabel>
					<CardActions>
						<Button
			...
			<OffCanvas
				setOpen={setOpenNewRom}
				isOpen={opentNewRom}
				titleId='New Rom'
				isBodyScroll
				placement='end'
				// className='offcanvas-newRom'
			>
				<OffCanvasHeader setOpen={setOpenNewRom}>
					<OffCanvasTitle id='NewRom'>New Rom</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						...
						<div className='col-12'>
							<FormGroup id='name' label='Tên tính năng'>
								<Input onChange={handleChangeRom} value={dataform.name} />
							</FormGroup>
						</div>
					</div>
				</OffCanvasBody>
				<div className='row m-0'>
					<div className='col-12 p-3'>
						...
						<Button color='info' className='w-100' onClick={() => postRom()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				setOpen={setOpenEditRom}
				isOpen={opentEditRom}
				titleId='Edit Rom'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenEditRom}>
					<OffCanvasTitle id='EditRom'>Edit Rom</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						...
						<Button color='info' className='w-100' onClick={() => editRom()}>
							Save
						</Button>
					</div>
				</div>
			...
			<OffCanvas
				setOpen={setOpenDeleteRom}
				isOpen={opentDeleteRom}
				titleId='Edit Rom'
				isBodyScroll
				placement='end'>
				<OffCanvasHeader setOpen={setOpenDeleteRom}>
					<OffCanvasTitle id='EditRom'>Delete Rom</OffCanvasTitle>
				</OffCanvasHeader>
				<OffCanvasBody>
					<div className='row g-4'>
						<div className='col-12'>
				...
				<div className='row m-0'>
					<div className='col-12 p-3'>
						...
						<Button color='waring' className='w-100' onClick={() => deleteRom()}>
							Delete
						</Button>
					</div>
				</div>
