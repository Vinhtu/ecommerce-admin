...
import {
	//  GetAdminNotifications, 
	 PutNotification } from '../../../redux/actions/notifications';
// eslint-disable-next-line react/prop-types
const CommonHeaderRight = ({ beforeChildren, afterChildren }) => {
	const dispatch = useDispatch();

	const { notificationAdminList } = useSelector((state) => state.notification);
	useEffect(() => {
	...
	const changeToStatusDone = (item) => {
		item.status = 'Done';
		dispatch(PutNotification(item._id, item));
	};
	let isPending = 0;
	if (notificationAdminList) {
		for (let i = 0; i < notificationAdminList.length; i += 1) {
			if (notificationAdminList[i].status === 'Pending') {
				isPending += 1;
			}
		}
	}
	const handleMarkAllRead = () => {
		for (let i = 0; i < notificationAdminList.length; i += 1) {
			if (notificationAdminList[i].status === 'Pending') {
				notificationAdminList[i].status = 'Done';
				dispatch(PutNotification(notificationAdminList[i]._id, notificationAdminList[i]));
			}
		}
	};
	return (
			...
			<OffCanvas
				...
				<OffCanvasHeader setOpen={setOffcanvasStatus}>
					<OffCanvasTitle id='offcanvasExampleLabel'>
						...
						{isPending > 0 ? (
							<Button
								color={isPending > 0 ? 'success' : 'primary'}
				...
				<OffCanvasBody>
					{notificationAdminList &&
						notificationAdminList.map((item) => {
							return (
								<Alert
									icon='ViewInAr'
									isLight
									...
									onClick={() =>
										item.status === 'Pending' ? changeToStatusDone(item) : null
									}>
									<h6 style={{ fontWeight: 'bold' }}>
										{item.title} -{' '}
										<span style={{ fontSize: 10 }}>{item.create_date}</span>
									</h6>
									{item.body}
								</Alert>
							);
